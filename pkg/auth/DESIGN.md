...
- `refresh_reuse:{refresh_token_hash}` 可选保存短 TTL 标记，用于识别 refresh token rotation 后的重放。
...
开启 rotation 后，旧 refresh token 被再次使用属于异常行为。推荐策略：

1. 如果 `refresh:{old_hash}` 不存在，但 `refresh_reuse:{old_hash}` 存在，返回 `ErrRefreshTokenReused`。
2. 触发 `RefreshReusedEvent`。
3. 可选吊销关联 session。当前先只返回错误和触发事件，是否吊销由业务层策略决定。
...
6. 如果开启 rotation，删除旧 refresh 记录，并写入 refresh_reuse 短 TTL 标记。
...
如果旧 refresh token 已不存在，但命中 `refresh_reuse:{hash}`，返回 `ErrRefreshTokenReused` 并触发 `RefreshReusedEvent`。
