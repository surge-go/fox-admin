...
Go 代码统一使用 `gofmt` 格式化，不手动对齐字段或 import。包名保持简短、小写，并按领域命名。系统数据库实体直接按领域命名，例如 `User`、`RoleMenu`、`LoginLog`。主键统一使用 `int64`，可选数据库字段优先使用指针类型。GORM 列名使用 `snake_case`。持久化实体的导出类型和字段应保留中文注释，便于后续生成文档和维护表结构。
